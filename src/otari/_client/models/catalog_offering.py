...
class CatalogOffering(BaseModel):
    ...
    credential: StrictStr = Field(description="Whose key serves it: `deployment` for a `providers:` instance the operator configured, `organization` for a key the viewer's organization holds.")
    ...
    def credential_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['deployment', 'organization', 'hosted']):
            raise ValueError("must be one of enum values ('deployment', 'organization', 'hosted')")
        return value
