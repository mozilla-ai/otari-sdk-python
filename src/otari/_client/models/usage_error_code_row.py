...
class UsageErrorCodeRow(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["error_class", "requests", "status_code"]

    @field_validator('error_class')
    def error_class_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['pricing', 'rate_limit', 'auth', 'provider_error', 'client_error', 'unknown']):
            raise ValueError("must be one of enum values ('pricing', 'rate_limit', 'auth', 'provider_error', 'client_error', 'unknown')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of UsageErrorCodeRow from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of UsageErrorCodeRow from a dict"""
        if obj is None:
            return None
